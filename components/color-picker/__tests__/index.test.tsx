...
describe('ColorPicker', () => {
  ...
  it('Should component custom trigger work', async () => {
    ...
    expect(document.body.querySelector('.ant-color-picker')).toBeTruthy();
    const hsbInputEls = document.body.querySelectorAll('.ant-color-picker-hsb-input input');
  ...
  it('Should popup open work', async () => {
    ...
    fireEvent.click(container.querySelector('.ant-color-picker-trigger')!);
    await waitFakeTimer();
    expect(document.body.querySelector('.ant-popover-hidden')).toBeTruthy();
  });
  ...
  it('Should disabled work', async () => {
    ...
    await waitFakeTimer();
    // Query document.body for portal-rendered popup content
    expect(document.body.querySelector('.ant-color-picker')).toBeFalsy();
  });
  ...
  it('Should allowClear and onClear work', async () => {
    ...
    await waitFakeTimer();
    // Query document.body for portal-rendered popup content
    expect(document.body.querySelector('.ant-color-picker-clear')).toBeTruthy();
    fireEvent.click(document.body.querySelector('.ant-color-picker-clear')!);
    expect(onClear).toHaveBeenCalledTimes(1);

    await waitFakeTimer();
    expect(
      document.body.querySelector('.ant-color-picker-alpha-input input')?.getAttribute('value'),
    ).toEqual('0%');
    expect(
      container.querySelector('.ant-color-picker-trigger .ant-color-picker-clear'),
    ).toBeTruthy();

    fireEvent.change(document.body.querySelector('.ant-color-picker-hex-input input')!, {
      target: { value: '#273B57' },
    });
    expect(
      document.body.querySelector('.ant-color-picker-alpha-input input')?.getAttribute('value'),
  ...
  it('Should preset color work', async () => {
    ...
    await waitFakeTimer();
    // Query document.body for portal-rendered popup content
    const presetsColors = document.body
      .querySelector('.ant-collapse-panel')
      ?.querySelectorAll('.ant-color-picker-presets-color')!;

    expect(document.body.querySelector('.ant-color-picker-presets')).toBeTruthy();
    ...
    expect(
      document.body.querySelector('.ant-color-picker-hex-input input')?.getAttribute('value'),
    ).toEqual('000000');
    expect(document.body.querySelectorAll('.ant-color-picker-presets-color')[0]).toHaveClass(
    ...
    expect(document.body.querySelectorAll('.ant-color-picker-presets-color')[9]).toHaveClass(
      'ant-color-picker-presets-color-checked',
    );
  ...
  describe('preset collapsed', () => {
    ...
    it('Should default collapsed work', async () => {
      render(<ColorPicker open presets={[recommendedPreset]} />);
      // Query document.body for portal-rendered popup content
      expect(document.body.querySelectorAll(selector)).toHaveLength(1);
    });
  ...
  it('Should format change work', async () => {
    ...
    await waitFakeTimer();
    // Query document.body for portal-rendered popup content
    expect(document.body.querySelector('.ant-color-picker-hex-input')).toBeTruthy();
    fireEvent.mouseDown(document.body.querySelector('.ant-color-picker-format-select')!);
    await waitFakeTimer();
    fireEvent.click(document.body.querySelector('.ant-select-item[title="HSB"]')!);
    await waitFakeTimer();
    expect(document.body.querySelector('.ant-color-picker-hsb-input')).toBeTruthy();

    fireEvent.mouseDown(document.body.querySelector('.ant-color-picker-format-select')!);
    await waitFakeTimer();
    fireEvent.click(document.body.querySelector('.ant-select-item[title="RGB"]')!);
    await waitFakeTimer();
    expect(document.body.querySelector('.ant-color-picker-rgb-input')).toBeTruthy();
  });

  it('Should hex input work', async () => {
    render(<ColorPicker open format="hex" />);
    // Query document.body for portal-rendered popup content
    fireEvent.change(document.body.querySelector('.ant-color-picker-hex-input input')!, {
      target: { value: 631515 },
    });
    expect(
      document.body.querySelector('.ant-color-picker-color-block-inner')?.getAttribute('style'),
    ).toEqual('background: rgb(99, 21, 21);');
  });

  it('Should rgb input work', async () => {
    render(<ColorPicker open format="rgb" />);
    // Query document.body for portal-rendered popup content
    const rgbInputEls = document.body.querySelectorAll('.ant-color-picker-rgb-input input');
  ...
  it('Should not trigger onChange when click clear after clearing', async () => {
    ...
    fireEvent.click(document.body.querySelector('.ant-popover .ant-color-picker-clear')!);
    expect(onChange).toHaveBeenCalledTimes(1);
  });
  ...
  it('Should fix hover boundary issues', async () => {
    ...
    await waitFakeTimer();
    doMouseMove(document.body as HTMLElement, 0, 999);
    // Query document.body for portal-rendered popup content
    expect(document.body.querySelector('.ant-popover-hidden')).toBeFalsy();
  ...
  it('Should work at dark mode', async () => {
    ...
    // Query document.body for portal-rendered popup content
    expect(document.body.querySelector('.ant-color-picker-presets-color-bright')).toBeFalsy();
  });
  ...
  it('Should showText work', async () => {
    ...
    fireEvent.mouseDown(document.body.querySelector('.ant-color-picker-format-select')!);
    await waitFakeTimer();
    fireEvent.click(document.body.querySelector('.ant-select-item[title="HEX"]')!);
    await waitFakeTimer();
  ...
  it('Should panelRender work', async () => {
    ...
    // Query document.body for portal-rendered popup content
    expect(document.body.querySelector('.custom-panel')).toBeTruthy();
    expect(document.body.querySelector('.ant-color-picker-inner-content')).toBeTruthy();
    expect(panelContainer).toMatchSnapshot();
  });

  it('Should panelRender with components work', async () => {
    const { container: componentContainer } = render(
      <ColorPicker
        open
        panelRender={(_, { components: { Picker, Presets } }) => (
          <div className="custom-panel-component">
            <Picker />
            <Presets />
          </div>
        )}
    ...
    // Query document.body for portal-rendered popup content
    expect(document.body.querySelector('.custom-panel-component')).toBeTruthy();
    expect(
      document.body.querySelector('.custom-panel-component .ant-color-picker-inner-content'),
    ).not.toBeTruthy();
    // Verify Picker component renders the color panel
    expect(
      document.body.querySelector('.custom-panel-component .ant-color-picker-panel'),
    ).toBeTruthy();
  ...
  it('Should null work as expect', async () => {
    ...
    render(<ColorPicker defaultValue={null} open />);
    // Query document.body for portal-rendered popup content
    expect(
      document.body.querySelector('.ant-color-picker-alpha-input input')?.getAttribute('value'),
    ).toEqual('0%');
  ...
  it('Should onChangeComplete work', async () => {
    ...
    render(<ColorPicker open onChangeComplete={handleChangeComplete} allowClear />);
    ...
    // Clear
    fireEvent.click(
      document.body.querySelector('.ant-color-picker-operation .ant-color-picker-clear')!,
    );
  ...
  it('Should disabledAlpha work', async () => {
    ...
    // Query document.body for portal-rendered popup content
    expect(
      document.body.querySelector('.ant-color-picker-slider-group-disabled-alpha'),
    ).toBeTruthy();
    expect(document.body.querySelector('.ant-color-picker-slider-alpha')).toBeFalsy();
    expect(document.body.querySelector('.ant-color-picker-alpha-input')).toBeFalsy();
  });
  ...
  it('Should defaultFormat work', () => {
    render(<ColorPicker open defaultFormat="hsb" />);
    // Query document.body for portal-rendered popup content
    expect(document.body.querySelector('.ant-color-picker-hsb-input')).toBeTruthy();
  });
  ...
  it('Controlled string value should work with allowClear correctly', async () => {
    ...
    // Clear - Query document.body for portal-rendered popup content
    fireEvent.click(document.body.querySelector('.ant-color-picker-clear')!);
    expect(
      container.querySelector('.ant-color-picker-trigger .ant-color-picker-clear'),
    ).toBeTruthy();
  ...
  describe('transparent to valuable', () => {
    ...
    it('init with hue', async () => {
      const onChange = vi.fn();
      render(<ColorPicker defaultValue={null} open onChange={onChange} />);
      // Query document.body for portal-rendered popup content
      doMouseMove(document.body as HTMLElement, 0, 50, 'ant-color-picker-slider-handle');
    ...
    it('init with alpha', async () => {
      ...
      doMouseMove(
        ...
        0,
        50,
        document.body.querySelectorAll<HTMLElement>('.ant-color-picker-slider-handle')[1]!,
      );
  ...
  it('onChangeComplete with default empty color should not be alpha', async () => {
    ...
    const handleChangeComplete = vi.fn();
    render(<ColorPicker open onChangeComplete={handleChangeComplete} />);

    // Move - Query document.body for portal-rendered popup content
    doMouseMove(document.body as HTMLElement, 50, 50);
    expect(handleChangeComplete).toHaveBeenCalledTimes(1);
  ...
  describe('controlled with `onChangeComplete`', () => {
    ...
    it('lock value', async () => {
      ...
      // Query document.body for portal-rendered popup content
      doMouseMove(document.body as HTMLElement, 0, 50, 'ant-color-picker-slider-handle', false);
      ...
      // Inline Color Block (locked) - in container
      expect(container.querySelectorAll('.ant-color-picker-color-block-inner')[0]).toHaveStyle({
        background: 'rgb(255, 0, 0)',
      });

      // Popup Color Block (follow operation) - in document.body
      expect(document.body.querySelectorAll('.ant-color-picker-color-block-inner')[1]).toHaveStyle({
  ...
  it('input precision', async () => {
    const onChange = vi.fn();
    render(<ColorPicker open onChange={onChange} />);
  ...
  describe('should disable colorInput', () => {
    it('Should defaultValue work with disabledFormat', async () => {
      ...
      // Query document.body for portal-rendered popup content
      expect(
        document.body.querySelector('.ant-color-picker-input-container .ant-select'),
      ).toBeFalsy();
    });

    it('Should rgb input work with disabledFormat', async () => {
      render(<ColorPicker open format="rgb" disabledFormat />);
      // Query document.body for portal-rendered popup content
      const rgbInputEls = document.body.querySelectorAll('.ant-color-picker-rgb-input input');
      fireEvent.change(rgbInputEls[0], {
        target: { value: 99 },
